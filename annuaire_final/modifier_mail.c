...
// on defini les variables //
    ...
    int n=0;                            //compteur qui permet de connaitre le nombre de caractere s�parateur
...
//-------------------------------------------------------------------------------------------------------------//
// on trouve l'endroit ou est marquer le mail (au 5 caracteres s�parateur) puis on r�ecrit une chaine avec le nouveau mail et on fini par r�crire la fin de chaine a partire du 6 s�parateur //
        if ((strstr(buffer, nom)!= NULL )&&(strstr(buffer, prenom)!= NULL )&&(strstr(buffer, emel)!= NULL )) {
